...
fn consume_ingredients(json_recipe: &Map<String, Value>, json_pantry: &mut Map<String, Value>) -> AppResult<()> {
    for (key, rval) in json_recipe.iter() {
        let pval = json_pantry.get_mut(key).unwrap();
        *pval = serde_json::value::to_value(pval.as_i64().unwrap() - rval.as_i64().unwrap())?;
    }
    Ok(())
}
...
async fn bake_cookies(TypedHeader(cookie): TypedHeader<Cookie>) -> AppResult<String> {
    ...
    output.insert(String::from("cookies"), serde_json::value::to_value(cookies)?);
    output.insert(String::from("pantry"), serde_json::value::to_value(json_pantry)?);
